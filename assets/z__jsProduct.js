...
Shopify.theme.jsProduct = {
  init: function init($section) {
    ...
    Shopify.theme.jsProduct = $.extend(this, Shopify.theme.getSectionData($section)); // function to check if browser is IE
    ...
    if ($productGallery) {
      ...
      if (Shopify.theme.jsProduct.enable_zoom) {
        document.addEventListener('lazyloaded', Shopify.theme.jsProduct.enableZoom);
      }
    ...
    if ($stickyElement && isScreenSizeLarge() && Shopify.theme.jsProduct.template == 'image-scroll') {
      Shopify.theme.jsProduct.enableStickyScroll($stickyElement, $productGallery);
    }
  ...
  enableStickyScroll: function enableStickyScroll($stickyElement, $productGallery) {
    ...
    var productImages = $productGallery.data('media-count'); //enable if more than 1 image is present
    ...
    if (thumbnailsEnabled == true && thumbnailsSliderEnabled == true && $slides.length > 1) {
      ...
      if (isScreenSizeLarge()) {
        if (thumbnailsPosition == 'right-thumbnails' || thumbnailsPosition == 'left-thumbnails') {
          ...
          var navHeight = $thumbnailProductGallery.height(); // Resize thumbnail gallery

          $(window).on('load', function () {
            var $productGalleryHeight = $productGallery.height();
          ...
          $productGallery.on('select.flickity', function () {
            ...
            if (flkty) {
              ...
              var scrollTop = _$selected.position().top;
              
              var scrollY = scrollTop + $thumbnailProductGallery.scrollTop() - (navHeight + navCellHeight) / 2;
          ...
          $thumbnailProductGallery.flickity({
            ...
            imagesLoaded: true,
            pageDots: false,
            prevNextButtons: $thumbnails.length > 0 ? arrowsEnabled : false,
  ...
  unload: function unload($section) {
    ...
    document.removeEventListener('lazybeforeunveil', Shopify.theme.jsProduct.enableZoom);
  }
};