  ...
  displayResults: function displayResults(results, $this) {
    ...
    if (results && results.length > 0) {
      $.each(results, function (index, result) {
        ...
        if(result.vendor != "Build A Bundle"){ // Hide Bundle Product
...
Shopify.theme.thumbnail = {
  ...
  showQuickShop: function showQuickShop() {
    ...
    $('body').on('click', '.js-quick-shop-link', function (e) {
      ...
      var productData = {
        ...
        product_discount_price: $(this).attr('data-discount-price'),
        check_subscription_active:$(this).attr('data-subscription-only'),
        subscription_status:$(this).attr('data-subscription-status'),
  ...
  afterContent: function afterContent($productForm, $notifyForm, productData, slide) {
    ...
    var badge = $('.jdgm-preview-badge[data-id="' + productData.product_id + '"]').first().clone();
    $('.quick-shop__title + div.jdgm-preview-badge').remove();
    $('.quick-shop__title').after(badge);
    
    
    // Update Subscription Price
    if(productData.check_subscription_active){
     var quickProductPrice = productData.check_subscription_active;
     var rc_quick_price = Shopify.formatMoney(productData.product_discount_price,productData.money_format);
     $('.rc_modal_price').html(rc_quick_price);
    }
  ...
  beforeOpen: function beforeOpen(productData, $quickshop) {
    ...
    if(productData.subscription_status == 'true' && productData.check_subscription_active == 'true'){
     $quickshop.find('.ck_autodeliver_price').addClass('has-padding-top');
     $quickshop.find('.ck_autodeliver_price').addClass('has-padding-bottom');
     $quickshop.find('.ck_autodeliver_price').removeClass('rc_hidden');
      
     $quickshop.find('.ck_onetime_price').removeClass('has-padding-top');
     $quickshop.find('.ck_onetime_price').removeClass('has-padding-bottom');
     $quickshop.find('.ck_onetime_price').addClass(' rc_hidden');
    }else{
     $quickshop.find('.ck_autodeliver_price').removeClass('has-padding-top');
     $quickshop.find('.ck_autodeliver_price').removeClass('has-padding-bottom');
     $quickshop.find('.ck_autodeliver_price').addClass('rc_hidden');
      
     $quickshop.find('.ck_onetime_price').addClass('has-padding-top');
     $quickshop.find('.ck_onetime_price').addClass('has-padding-bottom');
     $quickshop.find('.ck_onetime_price').removeClass(' rc_hidden');
    }
